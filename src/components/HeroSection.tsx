  ...
  return (
    <section className="relative overflow-hidden pt-16 pb-8 md:pt-28 md:pb-16">
      <div className="container grid items-center gap-4 lg:grid-cols-[1.1fr_0.9fr] lg:gap-8">
        {/* Mobile: card first, then text. Desktop: text first */}
        <div className="order-2 lg:order-1">
          ...
          >
            <div className="space-y-2">
              <h1 className="max-w-[700px] text-[2.5rem] font-black leading-[1.06] md:text-5xl lg:text-[4rem]">
                Оплата зарубежных
                {!isMobile ? (
                ...
                <span className="block mt-1 typing-cursor text-foreground/90 text-xl md:text-3xl lg:text-4xl">{displayText}</span>
              </h1>
              <p className="max-w-xl text-sm text-muted-foreground leading-relaxed md:text-base lg:text-lg">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-2">
              <motion.a
                ...
                target="_blank"
                rel="noreferrer"
                className="button-glow inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-2.5 font-bold text-primary-foreground md:px-6 md:py-3"
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.97 }}
              >
              ...
              <motion.a
                ...
                target="_blank"
                rel="noreferrer"
                className="button-secondary-glow inline-flex items-center justify-center gap-2 rounded-2xl border border-primary/25 bg-primary/10 px-5 py-2.5 font-bold text-foreground md:px-6 md:py-3"
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.97 }}
              >
              ...
              <Link to="/pricing">
                <motion.span
                  className="pricing-btn-glow inline-flex items-center justify-center gap-2 rounded-2xl bg-primary/20 border border-primary/40 px-5 py-2.5 font-bold text-primary md:px-6 md:py-3"
                  whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.97 }}
                >
        ...
        {/* Card — on mobile shows first (order-1) */}
        <motion.div
        ...
        >
          <img
            ...
            className="hero-float relative z-10 w-[200px] max-w-full md:w-[420px] card-glow-shadow"
            loading="eager"
          />
        </motion.div>
        ...
        <motion.div
          className="order-3 lg:col-span-2 lg:max-w-xl"
        ...
        >
          <div className="grid gap-2 grid-cols-3">
            {stats.map((stat, i) => (
              <motion.div
                key={stat.label}
                className="glass-card-glow rounded-2xl p-2.5 md:p-3.5"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
