        ...
        <div className="rounded-xl border border-border bg-card p-8 space-y-6">
          <div>
            ...
            <input type="number" min={1} value={amount} onChange={(e) => setAmount(Number(e.target.value))}
              className="w-full px-4 py-3 rounded-lg bg-secondary border border-border text-foreground text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
            ...
            <div className="flex gap-2">
              {currencies.map((c) => (
                <button key={c.code} onClick={() => setCurrency(c.code)}
                  className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-colors border ${currency === c.code ? "bg-primary text-primary-foreground border-primary" : "bg-secondary border-border text-secondary-foreground hover:bg-secondary/80"}`}>
                  {c.code} {c.label}
                </button>
              ))}
            </div>
          ...
          <div className="pt-4 border-t border-border">
            ...
            <a href="https://t.me/nowsub_ru" target="_blank" rel="noreferrer"
              className="block w-full text-center py-3 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground font-semibold transition-colors">
              Заказать
            </a>
          </div>
